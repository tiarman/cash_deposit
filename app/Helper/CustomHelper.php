...
//  const NidRegex = "/(?:\d{17}|\d{13}|\d{10})/";
  const URLWithHTTPSRegex = "/^https?:\\/\\/(?:www\\.)?[-a-zA-Z0-9@:%._\\+~#=]{1,256}\\.[a-zA-Z0-9()]{1,6}\\b(?:[-a-zA-Z0-9()@:%_\\+.~#?&\\/=]*)$/";
   ...
   * @return bool|string
   */
  public static function checkEventParticipant($participant = [],$event_id = null, $user_id =null){
    foreach($participant as $item){
      if(($item->job_event_id == $event_id) && (auth()->id()==$user_id)){
        return "Applied";
      }
    }
    return "Apply";
  ...
  public static function isInstituteTrainingProvider(): bool {
    if (auth()->user() && $institute = Institute::find(auth()->user()->institute_id)) {
      return $institute->institute_type_id == 1;
    }
    return false;
  }
}
