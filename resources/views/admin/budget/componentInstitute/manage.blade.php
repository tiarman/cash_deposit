        ...
        <div class="card-body">
          <section class="panel">
            <header class="panel-heading">
              <h2 class="panel-title">Manage Component Institute Budget</h2>
            </header>
            <div class="panel-body">
              @if(\App\Helper\CustomHelper::canView('List Of Component Institute Budget', 'Super Admin'))
                <div class="row">
                  <div class="col-lg-12 col-md-12 col-xl-12 text-right mb-3">
                    <a href="{{ route('admin.budget.component.institute.list') }}" class="brn btn-success btn-sm">List of Component Institute Budgets</a>
                  </div>
                </div>
              @endif
              ...
              <form action="{{ route('admin.budget.component.institute.store') }}" method="post">
                @csrf
                <input type="hidden" class="dt form-control" name="id" value="{{ $cib->id }}">
                <div class="row">
                  <div class="col-md-12">
                    <div class="form-row p-1">
                      <label class="control-label mr-3">Budget Type<span class="text-danger">*</span></label>
                      <label for="Monthly" class="control-label"> <input type="radio" name="type" @checked(old('type', $cib->type) == 'monthly') class="form-control p-2 ml-2 mr-2" value="monthly" id="Monthly">Monthly</label>
                      <label for="Yearly" class="control-label"> <input type="radio" name="type" @checked(old('type', $cib->type) == 'yearly') class="form-control p-2 ml-2 mr-2" value="yearly" id="Yearly">Yearly</label>
                      @error('component_id')
                    ...
                    <div class="form-group">
                      ...
                      <select name="fiscal_year_id" class="form-control @error('fiscal_year_id') is-invalid @enderror" required>
                        ...
                        @foreach($fsYears as $year)
                          <option value="{{ $year->id }}" @selected($year->id == old('fiscal_year_id', $cib->fiscal_year_id))>{{ $year->code }}</option>
                        @endforeach
                      </select>
                    ...
                    <div class="form-group">
                      ...
                      <select name="component_id" class="form-control @error('component_id') is-invalid @enderror" required>
                        ...
                        @foreach($budgets as $budget)
                          <option @if($budget->disabled) disabled
                                  @endif value="{{ $budget->id }}" @selected($budget->id == old('component_id', $cib->component_id))>{{ $budget->component->name }}
                    ...
                    <div class="form-group">
                      ...
                      <select name="institute_id" class="form-control @error('institute_id') is-invalid @enderror" required>
                        ...
                        @foreach($institutes as $institute)
                          <option value="{{ $institute->id }}" @selected($institute->id == old('institute_id', $cib->institute_id))>{{ $institute->name }}</option>
                        @endforeach
                      </select>
                      @error('institute_id')
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <input type="number" min="0" name="annual_budget" placeholder=" Annual Budget" autocomplete="off" required
                             value="{{ old('annual_budget', $cib->annual_budget) }}"
  ...
  <script>
    $(document).ready(function () {
      ...
      function getComponent() {
        ...
        var idDivision = $('select[name="fiscal_year_id"]').val();
        $this.html('');
        $this.html('<option value="">-- Select a component --</option>');
        if (idDivision!== null && idDivision!== '') {
          const oldCompId = '{{ old('component_id', $cib->component_budget_id) }}'
          $.ajax({
            ...
            success: function (result) {
              $.each(result.budgets, function (key, value) {
                $this.append('<option value="' + value.id + '" '+ ((Number(oldCompId) === Number(value.id)) ? "selected" : "") +' ' + (value.disabled ? "disabled" : "") + '>' + value.component.name + ' [' + value.availableBudget + ' TK]</option>');
              });
