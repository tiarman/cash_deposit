        ...
        <div class="card-body">
          <section class="panel">
            <header class="panel-heading">
              <h2 class="panel-title">Create Sub Component Institute Budget</h2>
            </header>
            <div class="panel-body">
              @if(\App\Helper\CustomHelper::canView('List Of Sub Component Institute Budget', 'Super Admin'))
                <div class="row">
                  <div class="col-lg-12 col-md-12 col-xl-12 text-right mb-3">
                    <a href="{{ route('admin.budget.sub.component.institute.list') }}" class="brn btn-success btn-sm">List of Sub Component Institute Budget</a>
                  </div>
                </div>
              @endif
                    ...
                    <div class="form-group">
                      ...
                      <select name="component_id" class="form-control @error('component_id') is-invalid @enderror" required>
                        ...
                        @foreach($budgets as $budget)
                          <option value="{{ $budget->id }}" @selected($budget->id == old('component_id'))>{{ $budget->component->name }} [ {{ $budget->annual_budget }} Tk. ]
                          </option>
                        @endforeach
                      </select>
                      @error('component_id')
                    ...
                    <div class="form-group">
                      ...
                      <select name="sub_component_id" class="form-control @error('sub_component_id') is-invalid @enderror" required>
                        ...
                        @foreach($subcomponents as $subcomp)
                          <option value="{{ $subcomp->id }}" @selected($subcomp->id == old('sub_component_id'))>{{ $subcomp->name }}</option>
                        @endforeach
                      </select>
        ...
        $.ajax({
          ...
          success: function (result) {
            $.each(result.budgets, function (key, value) {
              $this.append('<option value="' + value.id + '" data-component-id="'+value.component.id+'" '+(value.disabled ? "disabled" : "") +'>' + value.component.name + ' [' + value.availableBudget + ' TK]</option>');
            });
