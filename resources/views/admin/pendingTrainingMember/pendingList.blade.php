        ...
        <div class="card-body">
          <section class="panel">
            <header class="panel-heading">
              <h2 class="panel-title">Pending Training Members</h2>
            </header>
            <div class="panel-body">
              @if(session()->has('status'))
                {!! session()->get('status') !!}
              @endif
              @if(\App\Helper\CustomHelper::canView('Create Super Admin', 'Super Admin'))
                <div class="row">
                  <div class="col-lg-12 col-md-12 col-xl-12 text-right mb-3">
                    <a href="{{ route('admin.training.member.manage.approved') }}" class="brn btn-primary btn-sm">Accepted List</a>
                    <a href="{{ route('admin.training.member.manage.rejected') }}" class="brn btn-secondary btn-sm">Rejected List</a>
                  </div>
                </div>
              @endif
              <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                ...
                <tr>
                  ...
                  <th>Name</th>
                  <th>Details</th>
                  @if(\App\Helper\CustomHelper::canView('Manage Super Admin|Delete Super Admin', 'Super Admin'))
                ...
                @foreach($datas as $key => $val)
                  <tr class="@if(($key%2) == 0)gradeX @else gradeC @endif">
                    ...
                    <td class="p-1 text-center">
                      ...
                      {{--                      <button type="button" class="btn btn-info btn-primary" onclick="view({{ $val->id }})"></button>--}}
                    </td>
{{--                    <td class="p-1 ">{{ \App\Helper\CustomHelper::userRoleName($val) }}</td>--}}
                    ...
                    @if(\App\Helper\CustomHelper::canView('Manage Super Admin|Delete Super Admin', 'Super Admin'))
                      <td class="text-center hidden-phone p-1" width="200">
                        @if($val->status == \App\Models\User::$statusArrays[0])
                          @if(\App\Helper\CustomHelper::canView('Manage Super Admin', 'Super Admin'))
                            <span class="btn btn-sm btn-success btn-accept accept_{{ $val->id }}"
                                  style="cursor: pointer"
                                  data-id="{{ $val->id }}">Accept</span>
                          @endif
                          @if(\App\Helper\CustomHelper::canView('Manage Super Admin', 'Super Admin'))
  ...
  <script>
    $(document).ready(function () {
      ...
      $(document).on('click', '.btn-reject', function () {
        ...
        // }
        $.ajax({
          url: "{{ route('admin.ajax.update.training.member.status') }}",
          method: "post",
          dataType: "html",
          data: {'id': id, 'status': status},
          success: function (data) {
      ...
      $(document).on('click', '.btn-view', function () {
        ...
        $.ajax({
          ...
          success:function (data){
            ...
            $("#ps").html(data.ps);
            $("#po").html(data.po);
            // console.log(data.division.name);
