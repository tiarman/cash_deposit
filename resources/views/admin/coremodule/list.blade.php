                ...
                <div class="card-body">
                    <section class="panel">
                        <header class="panel-heading">
                            <h2 class="panel-title">List of Core Module</h2>
                        </header>
...
{{--                                    <a href="{{ route('admin.coremodule.create') }}" class="brn btn-success btn-sm">New Core Module</a>--}}
{{--                                </div>--}}
{{--                            </div>--}}
                            ...
                            <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                                ...
                                @foreach($data as $key => $val)
                                    <tr class="@if(($key%2) == 0)gradeX @else gradeC @endif">
                                        ...
                                        <td class="text-capitalize p-1" width="100">
                                            <div class="onoffswitch">
                                                <input type="checkbox" name="onoffswitch" class="onoffswitch-checkbox"
                                                       @if($val->status == \App\Models\CoreModule::$statusArrays[0])
                                                           checked
                                                       @endif
                                                       data-id="{{ $val->id }}"
                                        ...
                                        <td class="center hidden-phone p-1" width="50">
                                            <a href="{{ route('admin.coremodule.manage', $val->id) }}" class="btn btn-sm btn-success"> <i class="fa fa-edit"></i> </a>
{{--                                            <span class="btn btn-sm btn-danger btn-delete delete_{{ $val->id }}" style="cursor: pointer" data-id="{{ $val->id }}"><i class="fa fa-tresh"></i></span>--}}
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('change', 'input[name="onoffswitch"]', function () {
                ...
                $.ajax({
                    url: "{{ route('admin.ajax.update.coremodule.status') }}",
                    method: "post",
                    dataType: "html",
            ...
            $(document).on('click', '.yes-btn', function () {
                ...
                $.ajax({
                    url: "{{ route('admin.coremodule.destroy') }}",
                    method: "delete",
                    dataType: "html",
                    data: {id: pid},
                    success: function (data) {
