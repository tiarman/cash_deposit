        ...
        <div class="card-body">
          <section class="panel">
            <header class="panel-heading">
              <h2 class="panel-title">Sub Component list</h2>
            </header>
            <div class="panel-body">
              ...
              @if(\App\Helper\CustomHelper::canView('Create Sub Component', 'Super Admin'))
                <div class="row">
                  <div class="col-lg-12 col-md-12 col-xl-12 text-right mb-3">
                    <a href="{{ route('admin.subComponent.create') }}" class="brn btn-success btn-sm">New Sub Component</a>
                  </div>
                </div>
              @endif
              <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                ...
                <tr>
                  ...
                  @if(\App\Helper\CustomHelper::canView('Manage Sub Component|Delete Sub Component', 'Super Admin'))
                ...
                @foreach($datas as $key => $val)
                  <tr class="@if(($key%2) == 0)gradeX @else gradeC @endif">
                    ...
                    <td class="p-1">{{ $val->code }}</td>
                    <td class="p-1">{{ $val->creator?->name_en }}</td>
                    <td class="p-1">{{ $val->editor->name_en ?? 'N\A' }}</td>
                    @if(\App\Helper\CustomHelper::canView('Manage Sub Component|Delete Sub Component', 'Super Admin'))
                      <td class="center hidden-phone p-1" width="100">
                          @if(\App\Helper\CustomHelper::canView('Manage Sub Component', 'Super Admin'))
                            <a href="{{ route('admin.subComponent.manage', [$val->id]) }}"
                               class="btn btn-sm btn-success"> <i class="fa fa-edit"></i> </a>
                          @endif
                          @if(\App\Helper\CustomHelper::canView('Delete Sub Component', 'Super Admin'))
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h4>Delete Subcomponent</h4>
        </div>
        <div class="modal-body">
          <strong>Are you sure to delete this subcomponent?</strong>
        </div>
  ...
  <script>
    $(document).ready(function () {
      ...
      $(document).on('click', '.yes-btn', function () {
        ...
        $.ajax({
          url: "{{ route('admin.subComponent.destroy') }}",
          method: "DELETE",
          dataType: "html",
          data: {id: pid},
          success: function (data) {
