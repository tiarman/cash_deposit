        ...
        <div class="card-body">
          <section class="panel">
            <header class="panel-heading">
              <h2 class="panel-title">Upazila list</h2>
            </header>
            <div class="panel-body">
              ...
              @if(\App\Helper\CustomHelper::canView('Create Upazila', 'Super Admin'))
                <div class="row">
                  <div class="col-lg-12 col-md-12 col-xl-12 text-right mb-3">
                    <a href="{{ route('admin.upazila.create') }}" class="brn btn-success btn-sm">New Upazila</a>
                  </div>
                </div>
              @endif
              <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                ...
                <tr>
                  ...
                  @if(\App\Helper\CustomHelper::canView('Manage Upazila|Delete Upazila', 'Super Admin'))
                ...
                @foreach($datas as $key => $val)
                  <tr class="@if(($key%2) == 0)gradeX @else gradeC @endif">
                    ...
                    <td class="p-1 text-capitalize">{{ $val->district->name }}</td>
                    <td width="200" class="p-1">{{ date('F d, Y h:i A', strtotime($val->created_at)) }}</td>
                    @if(\App\Helper\CustomHelper::canView('Manage Upazila|Delete Upazila', 'Super Admin'))
                      <td class="center hidden-phone p-1" width="100">
                        @if(in_array($val->id, [1,2,3]))
                          @if(\App\Helper\CustomHelper::canView('Manage Upazila', 'Super Admin'))
                            <a href="#" style="cursor: not-allowed"
                               class="btn btn-sm btn-success"> <i class="fa fa-edit"></i> </a>
                          @endif
                          @if(\App\Helper\CustomHelper::canView('Delete Upazila', 'Super Admin'))
                            <span class="btn btn-sm btn-danger" style="cursor: not-allowed"><i class="fa fa-trash-o"></i></span>
                          @endif
                        @else
                          @if(\App\Helper\CustomHelper::canView('Manage Upazila', 'Super Admin'))
                            <a href="{{ route('admin.upazila.manage', [$val->id]) }}"
                               class="btn btn-sm btn-success"> <i class="fa fa-edit"></i> </a>
                          @endif
                          @if(\App\Helper\CustomHelper::canView('Delete Upazila', 'Super Admin'))
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <strong>Are you sure to delete this certificate.</strong>
        </div>
  ...
  <script>
    $(document).ready(function () {
      ...
      $(document).on('click', '.yes-btn', function () {
        ...
        $.ajax({
          url: "{{ route('admin.upazila.destroy') }}",
          method: "DELETE",
          dataType: "html",
          data: {id: pid},
          success: function (data) {
