        ...
        <div class="card-body">
          <section class="panel">
            <header class="panel-heading">
              <h2 class="panel-title">Fiscal Year list</h2>
            </header>
            <div class="panel-body">
              ...
              @if(\App\Helper\CustomHelper::canView('Create Fiscal Year', 'Super Admin'))
                <div class="row">
                  <div class="col-lg-12 col-md-12 col-xl-12 text-right mb-3">
                    <a href="{{ route('admin.fiscal.year.create') }}" class="brn btn-success btn-sm">New Fiscal Year</a>
                  </div>
                </div>
              @endif
              <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                ...
                <tr>
                  ...
                  <th>Status</th>
                  @if(\App\Helper\CustomHelper::canView('Manage Fiscal Year|Delete Fiscal Year', 'Super Admin'))
                ...
                @foreach($datas as $key => $val)
                  <tr class="@if(($key%2) == 0)gradeX @else gradeC @endif">
                    ...
                    <td class="p-1 text-capitalize">{{ $val->code }}</td>
                    ...
                    @if(\App\Helper\CustomHelper::canView('Manage Fiscal Year', 'Super Admin'))
                      <td class="text-capitalize p-1" width="100">
                        <div class="onoffswitch">
                          <input type="checkbox" name="onoffswitch" class="onoffswitch-checkbox"
                                 @checked($val->status == \App\Models\FiscalYear::$statusArrayss[1])
                                 data-id="{{ $val->id }}"
                    ...
                    @if(\App\Helper\CustomHelper::canView('Manage Fiscal Year|Delete Fiscal Year', 'Super Admin'))
                      <td class="center hidden-phone p-1" width="100">
                          @if(\App\Helper\CustomHelper::canView('Manage Fiscal Year', 'Super Admin'))
                            <a href="{{ route('admin.fiscal.year.manage', [$val->id]) }}"
                               class="btn btn-sm btn-success"> <i class="fa fa-edit"></i> </a>
                          @endif
                          @if(\App\Helper\CustomHelper::canView('Delete Fiscal Year', 'Super Admin'))
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h4>Delete Fiscal Year</h4>
        </div>
        <div class="modal-body">
          <strong>Are you sure to delete this Fiscal Year?</strong>
        </div>
  ...
  <script>
    $(document).ready(function () {
      ...
      $(document).on('change', 'input[name="onoffswitch"]', function () {
        ...
        $.ajax({
          url: "{{ route('admin.ajax.update.fiscal.year.status') }}",
          method: "post",
          dataType: "html",
      ...
      $(document).on('click', '.yes-btn', function () {
        ...
        $.ajax({
          url: "{{ route('admin.fiscal.year.destroy') }}",
          method: "DELETE",
          dataType: "html",
          data: {id: pid},
          success: function (data) {
