              ...
              @if(\App\Helper\CustomHelper::canView('Create Institute User', 'Super Admin'))
                <div class="row">
                  <div class="col-lg-12 col-md-12 col-xl-12 text-right mb-3">
                    <a href="{{ route('admin.institute.head.employee.create') }}" class="brn btn-success btn-sm">New Employee</a>
                  </div>
                </div>
              @endif
...
{{--                  <th>Address</th>--}}
                ...
                @foreach($datas as $key => $val)
                  <tr class="text-left @if(($key%2) == 0)gradeX @else gradeC @endif">
                    <td class="p-1">{{ ($key+1) }}</td>
                    <td class="p-1">{{ $val->name_en }}</td>
                    ...
                    <td class="p-1">{{ $val->email }}</td>
                      ...
                      @endif
                    @if(\App\Helper\CustomHelper::canView('Manage Institute User|Delete Institute User', 'Institute Head'))
                      <td class="center hidden-phone p-1" width="100">
                          {{-- @if(\App\Helper\CustomHelper::canView('Manage Institute User', 'Institute Head'))
                            <a href="{{ route('admin.institute.head.employee.manage', [$val->id]) }}"
                               class="btn btn-sm btn-success"> <i class="fa fa-edit"></i> </a>
                          @endif --}}

                          @if(\App\Helper\CustomHelper::canView('Delete Institute User', 'Institute Head'))
