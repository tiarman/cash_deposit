                ...
                <div class="card-body">
                    <section class="panel">
                        <header class="panel-heading">
                            <h2 class="panel-title">Manage Fiscal Period</h2>
                        </header>
                        <div class="panel-body">
                            @if(\App\Helper\CustomHelper::canView('List Of Fiscal Period', 'Super Admin'))
                                <div class="row">
                                    <div class="col-lg-12 col-md-12 col-xl-12 text-right mb-3">
                                        <a href="{{ route('admin.fiscalPeriod.list') }}" class="brn btn-success btn-sm">List of Fiscal Periods</a>
                                    </div>
                                </div>
                            @endif
                  ...
                  @csrf
                  <input type="hidden" class="dt form-control" name="id" value="{{ $fiscalPeriod->id }}">
                  <div class="row">
                      <div class="col-sm-12">
                          <div class="form-group">
                              ...
                              <select name="fiscal_year_id" class="form-control @error('fiscal_year_id') is-invalid @enderror" required>
                                  ...
                                  @foreach($fiscalYears as $fiscalYear)
                                      <option value="{{ $fiscalYear->id }}" @selected($fiscalYear->id == old('fiscal_year_id', $fiscalYear->id))>{{ $fiscalYear->code }}</option>
                                  @endforeach
                              </select>
                      ...
                      <div class="col-sm-6">
                          <div class="form-group">
                              ...
                              <input type="text" name="month_name" placeholder="Month Name" autocomplete="off"
                                     value="{{ old('month_name', $fiscalPeriod->month_name) }}"
                              ...
                              <select name="quarter_no" required class="form-control @error('quarter_no') is-invalid @enderror">
                                  ...
                                  @foreach(\App\Models\FiscalPeriod::$quarterNo as $quarter_no)
                                      <option value="{{ $quarter_no }}"
                                              @if(old('quarter_no', $fiscalPeriod->quarter_no) == $quarter_no) selected @endif>{{ ucfirst($quarter_no) }}</option>
                                  @endforeach
                              </select>
                              @error('quarter_no')
