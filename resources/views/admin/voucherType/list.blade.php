            ...
            <div class="panel-body">
              ...
              @if(\App\Helper\CustomHelper::canView('Create Voucher Type', 'Super Admin'))
                <div class="row">
                  <div class="col-lg-12 col-md-12 col-xl-12 text-right mb-3">
                    <a href="{{ route('admin.voucher.type.create') }}" class="brn btn-success btn-sm">New Voucher Type</a>
                  </div>
                </div>
              @endif
              <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                ...
                <tr>
                  ...
                  @if(\App\Helper\CustomHelper::canView('Manage Voucher Type|Delete Voucher Type', 'Super Admin'))
                ...
                @foreach($datas as $key => $val)
                  <tr class="@if(($key%2) == 0)gradeX @else gradeC @endif">
                    ...
                    <td class="p-1 text-capitalize">{{ $val->short_name }}</td>
                    <td class="p-1">{{ $val->name }}</td>
                    @if(\App\Helper\CustomHelper::canView('Manage Voucher Type|Delete Voucher Type', 'Super Admin'))
                      <td class="center hidden-phone p-1" width="100">
                          @if(\App\Helper\CustomHelper::canView('Manage VoucherType', 'Super Admin'))
                            <a href="{{ route('admin.voucher.type.manage', [$val->id]) }}"
                               class="btn btn-sm btn-success"> <i class="fa fa-edit"></i> </a>
                          @endif
                          @if(\App\Helper\CustomHelper::canView('Delete VoucherType', 'Super Admin'))
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h4>Delete Voucher Type</h4>
        </div>
        <div class="modal-body">
          <strong>Are you sure to delete this voucher type?</strong>
        </div>
  ...
  <script>
    $(document).ready(function () {
      ...
      $(document).on('click', '.yes-btn', function () {
        ...
        $.ajax({
          url: "{{ route('admin.voucher.type.destroy') }}",
          method: "DELETE",
          dataType: "html",
          data: {id: pid},
          success: function (data) {
