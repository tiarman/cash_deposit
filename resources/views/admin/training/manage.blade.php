        ...
        <div class="card-body">
          <section class="panel">
            <header class="panel-heading">
              <h2 class="panel-title">Manage training</h2>
            </header>
            <div class="panel-body">
              ...
              <form action="{{ route('admin.training.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                <input type="hidden" class="dt form-control" name="id" value="{{ $training->id }}">

                <div class="row">
                  @if(\App\Helper\CustomHelper::canView('List Of Training', 'Super Admin'))
                    <div class="col-sm-12">
                      <div class="form-group">
                        <label for="institute_id" class="control-label">Institute Name<span class="text-danger">*</span></label>
                        <select name="institute_id" id="institute_id" class="form-control @error('institute_id') is-invalid @enderror">
                          @foreach($institutes as $item)
                            <option {{ ($item->id==$training->institute_id) ? "selected" : "" }} value="{{ $item->id }}">{{ $item->name }}</option>
                          @endforeach
                        </select>
                      </div>
                    </div>
                      ...
                      <select name="type_id" required class="form-control @error('type_id') is-invalid @enderror">
                        ...
                        @foreach($trainingtype as $type)
                          <option value="{{ $type->id }}"
                                  @if(old('type_id', $training->type_id) == $type->id) selected @endif>{{ ucfirst($type->name) }}</option>
                        @endforeach
                      </select>
                      @error('type_id')
                ...
                <div class="row">
                    ...
                    <div class="form-group">
                      ...
                      <select name="participants[]" multiple class="form-control select2 @error('participants') is-invalid @enderror" required>
                        ...
                        @foreach($instituteTypes as $insType)
                          <option value="{{ $insType->id }}" @selected(in_array($insType->id,old('participants', $training->participants?->pluck('id')->toArray())?? []))>{{ $insType->name }}</option>
                        @endforeach
                      </select>
                      @error('participants')
                  ...
                  <div class="col-sm-3">
                    <div class="form-group">
                      ...
                      <input type="number" name="participant_limit" placeholder="Participant Limit" autocomplete="off"
                             value="{{ old('participant_limit', $training->participant_limit) }}"
                             class="form-control @error('participant_limit') is-invalid @enderror">
                      @error('participant_limit')
                      <strong class="text-danger">{{ $errors->first('participant_limit') }}</strong>
                      @enderror
                    </div>
                  </div>
                    ...
                    <div class="form-group">
                      ...
                      <select name="country_id" class="form-control @error('country_id') is-invalid @enderror" required>
                        ...
                        @foreach($countries as $country)
                          <option value="{{ $country->id }}" @selected($country->id == old('country_id', $training->country_id ?? 19))>{{ $country->name }}</option>
                        @endforeach
                      </select>
                      @error('country_id')
                    ...
                    <div class="form-group">
                      ...
                      <select name="division_id" class="form-control @error('division_id') is-invalid @enderror" required>
                        ...
                        @foreach($divisions as $division)
                          <option value="{{ $division->id }}" @selected($division->id == old('division_id', $training->division_id ?? $institute->division_id))>{{ $division->name }}</option>
                        @endforeach
                      </select>
                      @error('division_id')
                    ...
                    <div class="form-group">
                      ...
                      <select name="district_id" class="form-control @error('district_id') is-invalid @enderror" required>
                        ...
                        @foreach($districts as $district)
                          <option value="{{ $district->id }}" @selected($district->id == old('district_id', $training->district_id ?? $institute->district_id))>{{ $district->name }}</option>
                        @endforeach
                      </select>
                      @error('district_id')
                    ...
                    <div class="form-group">
                      ...
                      <select name="upazila_id" class="form-control @error('upazila_id') is-invalid @enderror" required>
                        ...
                        @foreach($upazilas as $upazila)
                          <option value="{{ $upazila->id }}" @selected($upazila->id == old('upazila_id', $training->upazila_id ?? $institute->upazila_id))>{{ $upazila->name }}</option>
                        @endforeach
                      </select>
                      @error('upazila_id')
                    ...
                    <div class="form-group">
                      ...
                      <select name="batch_creator_id" class="form-control @error('batch_creator_id') is-invalid @enderror" required>
                        ...
                        @foreach($batchcreators as $user)
                          <option value="{{ $user->id }}" @selected($user->id == old('batch_creator_id', $training->batch_creator_id))>{{ $user->name_en }}</option>
                        @endforeach
                      </select>
                    ...
                    <div class="form-group">
                      ...
                      <select name="batch_approver_id" class="form-control @error('batch_approver_id') is-invalid @enderror" required>
                        ...
                        @foreach($batchapprovers as $user)
                          <option value="{{ $user->id }}" @selected($user->id == old('batch_approver_id', $training->batch_approver_id))>{{ $user->name_en }}</option>
                        @endforeach
                      </select>
                  ...
                  <div class="col-md-12">
                    <div class="form-group">
                      ...
                      <textarea name="description" placeholder="Write description" autocomplete="off"
                                class="form-control summernote @error('description') is-invalid @enderror">{{ old('description', $training->description) }}</textarea>
                      @error('description')
                ...
                {{--                  @foreach($training_file->files as $file)--}}
                ...
                @foreach($training_file as $val)
                  <div class="col-md-12 mb-2">
                    <a href="{{ route('training.file.delete', ['trainingId'=>$training->id, 'id'=>$val->id]) }}" class="btn btn-danger">
