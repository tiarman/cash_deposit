        ...
        <div class="card-body">
          <section class="panel">
            <header class="panel-heading">
              <h2 class="panel-title">Voucher list</h2>
            </header>
            <div class="panel-body">
              ...
              @if(\App\Helper\CustomHelper::canView('Create Voucher', 'Super Admin'))
                <div class="row">
                  <div class="col-lg-12 col-md-12 col-xl-12 text-right mb-3">
                    <a href="{{ route('admin.voucher.create') }}" class="brn btn-success btn-sm">New Voucher</a>
                  </div>
                </div>
              @endif
              <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                ...
                <tr>
                  ...
                  @if(\App\Helper\CustomHelper::canView('Manage Voucher|Delete Voucher', 'Super Admin'))
                ...
                @foreach($datas as $key => $val)
                  <tr class="@if(($key%2) == 0)gradeX @else gradeC @endif">
                    ...
                    <td class="p-1">{{ $val->no }}</td>
                    <td class="p-1">{{ $val->year->code }}</td>
                    <td class="p-1">{{ $val->type->short_name }}</td>
                    <td class="p-1">{{ $val->institute->name }}</td>
                    <td class="p-1">{{ $val->amount }}</td>
                    @if(\App\Helper\CustomHelper::canView('Manage Voucher|Delete Voucher', 'Super Admin'))
                      <td class="center hidden-phone p-1" width="100">
                          @if(\App\Helper\CustomHelper::canView('Manage Voucher', 'Super Admin'))
                            <a href="{{ route('admin.voucher.manage', [$val->id]) }}"
                               class="btn btn-sm btn-success"> <i class="fa fa-edit"></i> </a>
                          @endif

                            <a href="{{ route('admin.voucher.view', [$val->id]) }}"
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <strong>Are you sure to delete this voucher?</strong>
        </div>
